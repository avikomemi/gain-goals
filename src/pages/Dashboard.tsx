...
const Dashboard = () => {
  ...
  const today = new Date().toLocaleDateString(lang === 'he' ? 'he-IL' : 'en-US', { weekday: 'long', day: 'numeric', month: 'long' });
  ...
  return (
    <div className="min-h-screen pb-20 px-4 pt-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        ...
        <h1 className="text-2xl font-bold mt-1">{t('dash.hello')}</h1>
      </motion.div>
      ...
      >
        <div className="text-right">
          <h2 className="text-lg font-bold text-primary-foreground">{t('dash.startWorkout')}</h2>
          <p className="text-sm text-primary-foreground/80">{t('dash.startWorkoutSub')}</p>
        </div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="grid grid-cols-3 gap-3 mb-6">
      ...
      {weightData.length > 1 && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-card rounded-xl p-4 border border-border mb-6">
          <h3 className="text-sm font-semibold mb-3">{t('dash.weightTrend')}</h3>
          <ResponsiveContainer width="100%" height={80}>
            <LineChart data={weightData}>
              <YAxis hide domain={['dataMin - 1', 'dataMax + 1']} />
              <Line type="monotone" dataKey="weight" stroke="hsl(160, 84%, 39%)" strokeWidth={2} dot={false} />
            </LineChart>
          </ResponsiveContainer>
        </motion.div>
      )}
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
        <h3 className="text-sm font-semibold mb-3">{t('dash.recentWorkouts')}</h3>
        {workoutHistory.length === 0 ? (
          <div className="bg-card rounded-xl p-6 border border-border text-center">
            <p className="text-muted-foreground text-sm">{t('dash.noWorkouts')}</p>
          </div>
        ) : (
              ...
              <div key={i} className="bg-card rounded-xl p-4 border border-border flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {new Date(w.date).toLocaleDateString(lang === 'he' ? 'he-IL' : 'en-US')} · {w.duration} {t('dash.minutes')}
                  </p>
                </div>
